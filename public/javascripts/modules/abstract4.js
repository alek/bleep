	...
	render() {	
		...
		for (var j=0; j<ymax; j++) {
			...
			for (var i=0; i<xmax; i++) {
				// result.push([(1-2*Math.random())*10, (1-2*Math.random())*10])
				result.push([(1-2*Math.random())*10, (1-2*Math.random())*10])
				// result.push([(1-2*Math.random())*Math.sqrt(i), (1-2*Math.random())*Math.sqrt(j)])
				//result.push([Math.pow(i,2), j])
			}
			this.vectorField.push(result)
		}
		// console.log(this.vectorField[0][0][1])
	}
	...
	update(event) {
		...
		for (var i=0; i<polygons.length; i++) {
			for (var j=0; j<polygons[i].points.length; j++) {
					var xoffset = Math.floor(polygons[i].points[j].x)
					var yoffset = Math.floor(polygons[i].points[j].y)
				// if (Math.random() < 0.9) {
					// polygons[i].points[j].x += (0.5-Math.random())*100
					// polygons[i].points[j].y += (0.5-Math.random())*100

					polygons[i].points[j].x += this.vectorField[i][j][0]
					polygons[i].points[j].y += this.vectorField[i][j][1]

					// polygons[i].points[j].x += (1-2*Math.random())*10
					// polygons[i].points[j].y += (1-2*Math.random())*10
